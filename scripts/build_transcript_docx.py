...
def build():
    ...
    info = [
        ...
        ("녹  음  자",
         "조합원 본인 ‘고영진’ — 이하 ‘갑’"),
        ("상  대  방",
         "① 김성수 이사 — 연신내지역주택조합 업무대행사 ‘주식회사 다원에코코리아’ 업무대행총괄 — 이하 ‘을₂’ / "
         "② 조합 안내직원(여) — 이하 ‘을₁’"),
        ("녹음 경위",
         "갑 고영진이 추가분담금 4,000만 원 납부 거부 및 ‘조합 가입계약의 해지(탈퇴)’를 "
         "위하여 연신내지역주택조합 사무실을 직접 방문하여 업무대행총괄 김성수 이사와 "
         "약 47분간 면담하였으며, 그 자리에서 갑이 구두로 탈퇴 의사를 표시한 데에 더하여 "
         "조합이 비치한 ‘조합원 탈퇴서’에 자필로 서명·날인하여 김성수 이사에게 직접 교부함으로써 "
         "본 일자로 가입계약이 적법하게 해지(탈퇴 완료)되었음. "
         "면담의 전 과정을 본인의 휴대전화로 녹음한 것이 본 음성파일임."),
        ("작 성 자", "고영진 (또는 대리 변호사)"),
    ]
    ...
    facts = [
        "1. 갑(고영진)은 2019년 12월경 (가칭)연신내지역주택조합 추진위원회(추진위원장 이수남) "
        "의 1·2차 조합원 모집 시 25평형으로 가입하고, 가입 이후 분담금 등 명목으로 "
        "총 90,000,000원(구천만 원)을 납입함. 본 조합의 업무대행사는 "
        "‘주식회사 다원에코코리아’(대표이사 김미란)이며, 자금관리신탁사는 "
        "‘한국자산신탁(주)’(대표이사 김규철)임.",
        "1-1. 본 조합이 은평구청에 등록한 모집공고문 변경(안)에 따르면, "
        "‘2022년 06월까지 약 95% 이상의 토지매매계약 및 소유권이전 완료 후 주택건설 "
        "사업계획승인 신청’할 것이 명시되어 있었으나, 면담 시점인 2023.3.15. 실제 "
        "토지매입률은 약 8%에 불과하여 약속 대비 약 44%포인트의 중대한 미달이 확인됨.",
        ...
        "6. 갑은 추가분담금 납부를 거부하고, 본 면담 자리에서 김성수 이사에게 ‘조합 가입계약의 "
        "해지(탈퇴)’ 의사를 구두로 명확히 표시한 데 더하여 조합이 비치한 "
        "‘조합원 탈퇴서’에 자필로 서명·날인하여 김성수 이사에게 직접 교부함. "
        "이에 김성수 이사가 곧바로 ‘해지 절차 및 환불 방법(2,500만 원 공제 후 6,500만 원 반환)’을 "
        "안내함으로써, 민법 제111조 제1항에 따라 위 해지의 의사표시(서면 탈퇴서 포함)가 "
        "상대방에게 도달하여 ‘2023년 3월 15일자로 가입계약은 적법하게 해지됨"
        "(즉, 동 일자로 탈퇴가 종국적으로 완료됨)’.",
    ...
    items = [
        ("1. 녹취록 인증 및 원본 보관",
         ...
         "원본 음성파일(m4a)은 휴대전화 원본·이동식 저장장치·클라우드 등에 "
         "최소 3부 이상 분산 보관할 것."),
        ("2. 내용증명 즉시 발송",
        ...
        ("4. 소송 청구 전략(예상)",
         ...
         "(2026.5.15. 기준 약 17,102,466원, 합계 약 107,102,466원). "
        ...
        ("6. 업무대행사(주식회사 다원에코코리아)에 대한 추가 청구 검토",
         "본 녹취 15:48 자인발언(‘홍보관 운영 등 부대비용 소진’) 등 분담금이 본래 목적인 "
         "토지매입이 아닌 운영비로 전용된 정황 및 모집공고상 95% 약속 위반 정황을 함께 "
         "고려할 경우, 업무대행사 자체에 대하여 「표시·광고의 공정화에 관한 법률」 제10조에 "
         "따른 손해배상 청구 등 회사 단위의 추가 책임 추궁을 검토할 수 있음. "
         "임원 개인에 대한 책임 추궁은 본 1차 절차에서는 자제하되, 회사 측의 비협조적 대응이 "
         "장기간 지속될 경우 그 시점에 별도 검토."),
    ]
    for h, body in items:
