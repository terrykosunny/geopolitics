...
def build():
    ...
    info = [
        ...
        ("상  대  방",
         "① 조합 안내직원(여) — 이하 ‘을₁’ / ② 조합 팀장(남, 성명 미상) — 이하 ‘을₂’"),
        ("녹음 경위",
         "갑(甲) 고영진이 추가분담금 4,000만 원 납부 거부 및 조합 탈퇴 의사 표명을 위해 "
         "연신내지역주택조합 사무실을 직접 방문한 자리에서, 면담의 전 과정을 휴대전화로 녹음한 것임."),
        ("작 성 자", "고영진 (또는 대리 변호사)"),
    ]
    ...
    facts = [
        "1. 갑(고영진)은 2019년 12월경 연신내지역주택조합 1·2차 조합원 모집 시 25평형으로 가입함.",
        ...
        "6. 갑은 추가분담금 납부를 거부하고 본 면담에서 명시적으로 ‘탈퇴 의사’를 표명함.",
        "7. 조합 측은 갑의 납입금 중 2,500만 원을 공제한 6,500만 원을 환급하되, "
        "그 시기는 ‘대체 조합원 모집 시(2024년 3월 ~ 8월 중)’로 한정한다고 안내함.",
    ...
    # 2부
    add_h2(doc, "【제2부 — 사업 진행 경과 설명】 03:17 ~ 16:00")
    ...
    # 4부 (핵심)
    add_h2(doc, "【제4부 — 갑의 탈퇴 의사 명시적 표명】 24:33 ~ 25:30  ★ 핵심 ★")
    add_issue_header(doc, "▣ 쟁점 ④ — 갑의 탈퇴 의사표시 (구두)")
    add_dialogue(doc, "24:33", "갑  ",
                 ...
                 highlight=True)
    add_note(doc,
             "갑이 본 면담일 ‘구두로 탈퇴 의사’를 명시적으로 표시함. "
             "면담 이전 ‘월요일’ 전화 문의 시점에 이미 조합 측이 환불 의사를 통지한 사실도 확인됨. "
             "민법 제111조에 의한 의사표시 도달로 ‘탈퇴 효력’ 발생 주장 가능.")
    ...
    items = [
        ...
        ("2. 내용증명 즉시 발송",
         "탈퇴 의사표시 및 납입금 전액 반환 청구를 담은 ‘내용증명’을 즉시 발송하여 시효 중단 및 "
         "지연이자(상사법정이율 연 6%) 기산점을 확보할 것. 14일 이내 미반환 시 즉시 소제기 예고."),
        ("3. 조합 측 자료 요구",
         "조합 규약 사본, 가입계약서 사본(특히 분담금 납부 시점 조항), 추가분담금 의결 총회결의서, "
         "분담금 사용 회계내역(특히 ‘홍보관 운영비’ 항목) 등을 정식으로 요구할 것."),
        ("4. 소송 청구 전략(예상)",
         "주위적 청구: 부담금반환 청구(납입금 전액). "
